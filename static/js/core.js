...
(function (){
  ...
  var _yam = function (callback){
    return yam._init().then(callback);
  }

  var _init = function (){
    if (!yam._pinit){
      yam._pinit = yam.promise.when(
        'currentUser'
      , 'currentNetwork'
      , 'init'
      , 'dom');
  ...
  if (typeof yam == 'undefined') {
    ...
    */
    ...
    var nyam = _yam, orig = yam;

    // nyam is the redefined version of yam. This is to account for loading problems
    // that can occur when yam is errantly defined as an empty object.
    nyam._init = _init;

    for (var p in orig){
      if (orig.hasOwnProperty(p)){ nyam[p] = orig[p]; }
    }
...
yam.core = new (function (yam) {
  ...
  */
  ...
  /**
    ...
    @param   {Object} [klass]    The superclass that is to be extended
    @param   {Array}  [mixins]   An array of mixins to be mixed in to the class definition
    @param   {Object} definition Normally an object literal, defines methods on the class
    @returns {Function} Returns a class.
    
    @example
    // normal example
    yam.ctor('bing.bong.Foo', {
      init: function (a, b){ console.log(a, b); }
    });

    var foo = new bing.bong.Foo(1, 2);
    ...
    // subclass example
    yam.ctor('bing.bong.Bar', bing.bong.Foo, {
      init: function (a, b){
        this._super(a, b);
    ...
    // mixin example
    yam.ns('bing.bong._Mixin', {
      yo: function (){ console.log('yo'); }
    });

    yam.ctor('bing.bong.Baz', bing.bong.Bar, [bing.bong._Mixin], {
      hey: function (){ console.log('hey'); }
    });

    var baz = new bing.bong.Baz(1, 2);
    baz.yo();
    baz.hey();
  ...
  */
  this.ctor = (function () {
    ...
    return function () {
      ...
      var ctor = function () {
        // Construct if we aren't connecting the prototype chain
        if (!_construct){ return; }
      ...
      if (!base) {
        ...
        var createMethodWithSuper = function (prop, func) {
          return function () {
            // Maintain the proper state of _super throughout the call stack.
            var tmp = this._super;
  ...
  */
  ...
  this.profile = (function() {
    var startTime, profileTime, elapsed;

    return function() {
      var args = Array.prototype.slice.call(arguments);
      if(!profileTime) {
        startTime = profileTime = new Date();
        elapsed = 0;
      } else {
        elapsed = new Date() - profileTime;
      }
      args.unshift('profile: ' + elapsed + ' ' +  (new Date() - startTime) + ': ')
      yam.log.apply(yam, args);
  ...
  this.setCurrentUser = function (user){
    if (user && !user.declaredClass){
      // If models are not included with core then this could be a problem,
      // and we may have to think of something else. - Bob
      yam.require('yam.model.User');
      user = yam.model.User.save(user);
    }
    if (user){
      if (user.attachments){ user.createMessageTypes(); }

      yam._init().satisfy('currentUser', user);
    }
  ...
  this.setCurrentNetwork = function (network){
    if (network && !network.declaredClass){
      // If models are not included with core then this could be a problem,
      // and we may have to think of something else. - Bob
      yam.require('yam.model.Network');
      network = yam.model.Network.save(network);
    }
    if (network){
      yam._init().satisfy('currentNetwork', network);
    }
    return this._currentNetwork = network;
  }
